...
class Annotator:
    ...
    def __init__(self, im, line_width=None, font_size=None, font='Arial.ttf', pil=True, centroid=False):
        assert im.data.contiguous, 'Image not contiguous. Apply np.ascontiguousarray(im) to Annotator() input images.'
        self.pil = pil
        self.centroid = centroid  # when True, draw only centroids (no labels)
        if self.pil:  # use PIL
    ...
    def box_label(self, box, label='', color=(128, 128, 128), txt_color=(255, 255, 255)):
        # Draw either a bounding box with label (default) or a centroid marker (lighter)
        x1, y1, x2, y2 = box
        if self.centroid:
            # Draw centroid only (no label)
            cx = int((x1 + x2) / 2)
            cy = int((y1 + y2) / 2)
            r = max(4, int(((abs(x2 - x1) + abs(y2 - y1)) / 8)))
            if self.pil or not is_ascii(label):
                self.draw.ellipse((cx - r, cy - r, cx + r, cy + r), outline=color, width=self.lw)
            else:
                cv2.circle(self.im, (cx, cy), r, color, thickness=self.lw, lineType=cv2.LINE_AA)
            return

        # Default behavior: draw bounding box and optional label
        if self.pil or not is_ascii(label):
